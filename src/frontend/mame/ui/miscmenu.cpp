...
void ui_menu_custom_button::populate(float &customtop, float &custombottom)
{
	std::string subtext;
	ioport_field *field;
	int menu_items = 0;
	...
	for (auto& pair : machine().ioport().ports() )
	{
		...
		for (field = port->fields().first(); field != NULL; field = field->next())
		{
			...
			if (name != NULL && type >= IPT_CUSTOM1 && type < IPT_CUSTOM1 + MAX_CUSTOM_BUTTONS)
			{
				...
				item_append(_(name), subtext.c_str(), 0, (void *)&machine().ioport().m_custom_button[player][type]);
