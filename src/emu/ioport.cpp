...
void ioport_list::append_custom(device_t &device, std::string &errorbuf)
{
	...
	for (auto &port : *this)
	{
		for (field = port.second->fields().first(); field != NULL; field = field->next())
		{
...
void ioport_field::frame_update(ioport_value &result)
{
	...
	{
		u16 button_mask = 1 << (this->type() - IPT_BUTTON1);
...
#endif	// if the state changed, look for switch down/switch up
...
g_profiler.start(PROFILER_INPUT);
	...
	for (auto &port : m_portlist)
	{
		...
		playback_port(*port.second);
		record_port(*port.second);
